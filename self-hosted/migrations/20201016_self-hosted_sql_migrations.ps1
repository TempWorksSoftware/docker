...
SELECT 1 from dbo.ApiScopes WHERE name='indeed-webhook'
'@
$sqlMigrationQuery1 = @'
...
SELECT 1 ,
                     'Allows Indeed access to create order candidates' ,
                     'Indeed Webhook' ,
                     0 ,
                     'indeed-webhook' ,
                     0 ,
                     0
'@
Apply-SqlMigration $loginServerConnectionString "Add indeed-webhook ApiScope to login database" $sqlMigrationTestQuery1 $sqlMigrationQuery1
