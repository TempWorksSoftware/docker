...
$sqlMigrationTestQuery = "SELECT TOP 1 1 FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SocialMediaIdentityProvider]') AND type in (N'U')"

$sqlMigrationQuery = @"
CREATE TABLE [dbo].[SocialMediaIdentityProvider]
    (
        Id BIGINT NOT NULL IDENTITY PRIMARY KEY,
        Name NVARCHAR(100) NOT NULL,
        AuthorityUrl NVARCHAR(1000) NOT NULL,
...
"@
Apply-SqlMigration $loginServerConnectionString "Create SocialMediaIdentityProvider table" $sqlMigrationTestQuery $sqlMigrationQuery
