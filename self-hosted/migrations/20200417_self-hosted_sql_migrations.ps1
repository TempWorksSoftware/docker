...
SELECT 1 FROM sys.columns c WHERE c.object_id = object_id('ServiceRepTokens') AND c.name = 'SubTenantId';
'@
$sqlMigrationQuery1 = @'
ALTER TABLE dbo.ServiceRepTokens
ADD SubTenantId INT NULL
        REFERENCES dbo.SubTenants ( SubTenantId );
'@
Apply-SqlMigration $loginServerConnectionString "SubTenant schema changes for service rep tokens" $sqlMigrationTestQuery1 $sqlMigrationQuery1

$sqlMigrationTestQuery2 = @'
SELECT 1
FROM INFORMATION_SCHEMA.ROUTINES r
WHERE r.ROUTINE_DEFINITION LIKE '%subtenant%' 
...
Apply-SqlMigration $loginServerConnectionString "SubTenant schema changes for GetTokenByAccountSid" $sqlMigrationTestQuery2 $sqlMigrationQuery2
